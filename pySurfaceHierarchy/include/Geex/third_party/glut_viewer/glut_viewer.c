...
// CJ
static GlutViewerSpecialKeyFunc special_key_func[50];
static GlutViewerSpecialKeyFunc special_key_up_func[50];
char input_fn[1024];

static GLboolean caps[8] = {GL_FALSE, GL_FALSE, GL_FALSE, GL_FALSE, GL_FALSE, GL_FALSE, GL_TRUE, GL_TRUE} ;
static int nb_caps = 8 ;

static float exposure = 0.5 ;

static char* title = "g33>|< Viewer";

static int skybox_tex ;

//<<<<<
static float pickp_x;
static float pickp_y;
static GlutViewerKeyFunc my_idle_func;
//>>>>>
...
/*
 * The ture rotation compound by multiply two quaternions	- CJ
 * q = x*i + y*j + z*k + s
 */
void mul_quats(float *q1, float *q2, float *dest);
...
int glut_viewer_W =1000, glut_viewer_H = 1000;
...
static float eye_pos[3] = {0.0, 0.0, 5.0};
static float up_dir[3] = {0.0, 1.0, 0.0};
...
void glut_viewer_get_m_screen_size(int* w, int* h){
...
void glut_viewer_save_transform_for_picking() {
	...
	//printf("translation: %lf %lf %lf %lf\n", modelview_save[12], modelview_save[13], modelview_save[14], modelview_save[15]);
}

void glut_viewer_get_saved_transform(float* rotation, float* xlat, float* scale){
	int i;
	for (i = 0; i < 4; i++)
	{
		rotation[i] = cur_rot[i];
	}
	for (i = 0; i < 3; i++)
	{
		xlat[i] = cur_xlat[i];
	...
	*scale = zoom;
	...
	//return transform_saved;
}

void glut_viewer_set_saved_transform(float* rotation, float* xlat, float scale){
	...
	for (i = 0; i < 4; i++)
	{
		cur_rot[i] = rotation[i];
	}

	zoom = scale;
}

void glut_viewer_get_saved_scale(float *scale)
{
	*scale = zoom;
}

void glut_viewer_get_saved_rotation(float* rotation)
{
	for (int i = 0; i < 4; i++)
	{
		rotation[i] = cur_rot[i];
...
void glut_viewer_get_picked_pos2d(GLdouble* pos){
	pos[0] = pickp_x;
	pos[1] = pickp_y;
}
...
static void save_picked_ray(int x_in, int y_in) {
    ...
    if(transform_saved) {
         ...
         */
        gluProject(x,y,0.0,modelview_save, project_save, viewport_save, &(temp[0]), &(temp[1]), &(temp[2])) ;

        gluUnProject(x,y, temp[2], modelview_save, project_save, viewport_save, &(ray_p_save[0]), &(ray_p_save[1]), &(ray_p_save[2])) ;
		//gluUnProject(x, y, 0, modelview_save, project_save, viewport_save, &(ray_p_save[0]), &(ray_p_save[1]), &(ray_p_save[2]));
        gluUnProject(x,y, 1.0, modelview_save, project_save, viewport_save, &(ray_v_save[0]), &(ray_v_save[1]), &(ray_v_save[2])) ;
        for(i=0; i<3; i++) {
            ray_v_save[i] -= ray_p_save[i] ;
        }
        glReadPixels(x,y,1,1,GL_DEPTH_COMPONENT, GL_FLOAT, &z);
        gluUnProject(x,y,z, modelview_save, project_save, viewport_save, &(ray_p3d_save[0]), &(ray_p3d_save[1]), &(ray_p3d_save[2])) ;
        ray_hit_background = (z == 1.0) ;
    }
}


void glut_viewer_screen_to_space(int x_in, int y_in, GLdouble* pt, GLdouble* ray){
	...
	//called in display before draw foreground, do not need to call it again
    ...
    if(transform_saved) {
        ...
        gluUnProject(x,y, temp[2], modelview_save, project_save, viewport_save, &(pt[0]), &(pt[1]), &(pt[2])) ;
        gluUnProject(x,y, 1.0, modelview_save, project_save, viewport_save, &(ray[0]), &(ray[1]), &(ray[2])) ;
        for(i=0; i<3; i++) {
            ray[i] -= pt[i] ;
        }
    }
...
void glut_viewer_space_to_screen(GLdouble* pt, GLdouble* pt_s){
	
	if(transform_saved){
		gluProject(pt[0], pt[1], pt[2], modelview_save, project_save, viewport_save, &pt_s[0], &pt_s[1], &pt_s[2]);
	}
}

void glut_viewer_set_rotation(float a[3], float theta, float m[4][4])
{
	float cur_quat[4];
	axis_to_quat(a, theta, cur_quat);
	build_rotmatrix(m, cur_quat);
}

void glut_viewer_set_rotation_using_quaternion(float delta_rot[4], float m[4][4])
{
	build_rotmatrix(m, delta_rot);
}

void glut_viewer_set_matrix(float a[3], float theta, float b[3], float new_quat[4], float m[4][4])
{
	// reference: http://run.usc.edu/cs520-s12/quaternions/quaternions-cs520.pdf
	float cur_quat[4];
	axis_to_quat(a, theta, cur_quat);
	mul_quats(cur_quat, b, new_quat);
	build_rotmatrix(m, new_quat);
}

void glut_viewer_get_rotation_quat(int x0, int y0, int x1, int y1, float *cur_rot)
{
	float cur_delta_rot[4];
	int W = window_w;
	int H = window_h;

	trackball(cur_delta_rot,
		(2.0 * x1 - W) / W,
		(H - 2.0 * y1) / H,
		(2.0 * x0 - W) / W,
		(H - 2.0 * y0) / H
	...
	add_quats(cur_delta_rot, cur_rot, cur_rot);
}

void glut_viewer_set_camera(float eye[3], float up[3])
{
	for (int i = 0; i < 3; i++)
	{
		eye_pos[i] = eye[i];
...
void glut_viewer_get_saved_matrix(float * modelViewMatrix, float * projectionMatrix)
{
	if (transform_saved)
	{
		for (int i = 0; i < 16; i++)
		{
			modelViewMatrix[i] = modelview_save[i];
			projectionMatrix[i] = project_save[i];
		}
	}
}

void glut_viewer_get_input_file_name(char *fn)
{
	strcpy(fn, input_fn);
}
...
// 	int my = -(int)(3000.0 * fy / l) ;
	...
	result = mouse_func(x, y, button, event) ;
    glutPostRedisplay() ;
    return result ;
}


//Switch for multi-motion mode. When on, the mouse events should be passed.
static int multi_touch_mode = 0;

static void mouse(int button, int state, int x, int y) {

	/*auto save the current position of mouse.*/
	pickp_x = x;
	pickp_y = y;
	...
	//check if multi-motion mode is on
	if (multi_touch_mode)
	{
		return;
	}
...
//         return ;
//     }
	if(glutGetModifiers() != GLUT_ACTIVE_CTRL && call_mouse_func(x,y,button,state ? GLUT_VIEWER_UP : GLUT_VIEWER_DOWN))  //to block the left button
	{
		return ;
	}
    mode = NONE ;
...
static void motion(int x, int y) {
	...
	//Pan Hao /*auto save the current position of mouse.*/
	pickp_x = x;
	pickp_y = y;
    ...
    case PAN: {
        if(!glut_viewer_is_enabled(GLUT_VIEWER_ROTATE_LIGHT)) {
			...
			cur_xlat[0] -= 4.5 * delta_x;
			cur_xlat[1] -= 3.0 * delta_y;
        }
    } break ;
    case ZOOM: {
       if(!glut_viewer_is_enabled(GLUT_VIEWER_ROTATE_LIGHT)) {
...
static void draw_foreground() {
    ...
    if(glut_viewer_is_enabled(GLUT_VIEWER_SHOW_HELP)) {
        ...
        if(glut_viewer_is_enabled(GLUT_VIEWER_HDR)) {
	   glut_viewer_printf("Exposure: [<]%f[>]", exposure) ;
	}
    }
...
static float bkg1[3] = { 255 / 255.0, 255 / 255.0, 255 / 255.0 };
static float bkg2[3] = { 25 / 255.0, 40 / 255.0, 150 / 255.0 };
...
static void display(void)
{
	...
	/* if selection mode, setup the pick matrix. by hp */
	//3D picking
	glGetIntegerv(GL_RENDER_MODE,&render_mode);
	glGetIntegerv(GL_VIEWPORT, cur_viewport);
	if (render_mode == GL_SELECT)
	{
		gluPickMatrix(pickp_x, cur_viewport[3] - pickp_y, 10, 10, cur_viewport);
	}
...
void save_scene_geex(){
	...
	FILE *out;
	out = fopen("camera_config_geex.txt", "w");
	fprintf(out, "%f %f %f %f\n", cur_rot_light[0], cur_rot_light[1], cur_rot_light[2], cur_rot_light[3]);
	fprintf(out, "%f %f %f\n", cur_xlat[0], cur_xlat[1], cur_xlat[2]);
	fprintf(out, "%f\n", zoom);
	fprintf(out, "%f %f %f %f\n",cur_rot[0],cur_rot[1],cur_rot[2],cur_rot[3]);
	fprintf(out, "%d %d", glut_viewer_W, glut_viewer_H);
	fclose(out);
}

void load_scene_geex(){
	int ww, wh;
	FILE *in;
	in = fopen("camera_config_geex.txt","r") ;
	fscanf(in, "%f %f %f %f\n", &cur_rot_light[0],&cur_rot_light[1],&cur_rot_light[2],&cur_rot_light[3]);
	fscanf(in, "%f %f %f\n", &cur_xlat[0],&cur_xlat[1],&cur_xlat[2]);
	fscanf(in, "%f\n",&zoom);
	fscanf(in, "%f %f %f %f\n",&cur_rot[0],&cur_rot[1],&cur_rot[2],&cur_rot[3]);
...
// CJ
static void specialkeyboard(unsigned char c, int x, int y)
{
#ifdef WITH_ANTTWEAKBAR
	if (glut_viewer_is_enabled(GLUT_VIEWER_TWEAKBARS) && TwEventKeyboardGLUT(c, x, y)) {
		glutPostRedisplay();
...
static void specialkeyboardUp(unsigned char c, int x, int y)
{
#ifdef WITH_ANTTWEAKBAR
	if (glut_viewer_is_enabled(GLUT_VIEWER_TWEAKBARS) && TwEventKeyboardGLUT(c, x, y)) {
		glutPostRedisplay();
	...
	if (special_key_up_func[c] != NULL)
	{
		special_key_up_func[c]();
		glutPostRedisplay();
	}
	glutPostRedisplay();
}
...
#ifdef WITH_HDR
    exposure /= 2.0 ;
    glut_viewer_hdr_set_exposure(exposure) ;
#endif
}
...
//<<<<<
void glut_viewer_set_idle_func(GlutViewerDisplayFunc f){
	my_idle_func = f;
}
//>>>>>
...
static void init_keys_if_needed() {
    ...
    if(first) {
		...
		for (i = 0; i < 50; i++)
		{
			special_key_func[i] = NULL;
			special_key_up_func[i] = NULL;
		}
    } 
}
...
// CJ
void glut_viewer_add_special_key_func(char key, GlutViewerKeyFunc f)
{
	init_keys_if_needed();
	if (key >= 100)
	{
	...
	special_key_func[key] = f;
}
void glut_viewer_add_special_key_up_func(char key, GlutViewerKeyFunc f)
{
	init_keys_if_needed();
	if (key >= 100)
	{
...
//Multi-touch callbacks for zooming.

static int multi_touch_buttons[2] = { -1, -1 };
static int multi_touch_button_coords[4] = { 0, 0, 0, 0 };


void MultiTouchButton(int device, int x, int y, int button, int status){

	//printf("Device: %d, button: %d, status: %d\n", device, button, status);

	if (status == GLUT_DOWN)
	{
		if (multi_touch_buttons[0]<0)
		{
			multi_touch_buttons[0] = device;
			multi_touch_button_coords[0] = x;
			multi_touch_button_coords[1] = y;
		}
		else if (multi_touch_buttons[1]<0)
		{
			multi_touch_buttons[1] = device;
			multi_touch_button_coords[2] = x;
			multi_touch_button_coords[3] = y;
		}

		if (multi_touch_buttons[0]>-1 && multi_touch_buttons[1]>-1)
		{
			multi_touch_mode = 1;
		}
	}
	...
	{
		if (device == multi_touch_buttons[0])
		{
			multi_touch_buttons[0] = -1;
			multi_touch_mode = 0;
		}
		else if (device == multi_touch_buttons[1])
		{
			multi_touch_buttons[1] = -1;
			multi_touch_mode = 0;
		}
	}
}

void MultiTouchMotion(int button, int x, int y){
	//printf("Button: %d  Coords (%d,%d)\n", button, x, y);


	if (multi_touch_buttons[0]>-1 && multi_touch_buttons[1]>-1)
	{
		float old_diag_len = sqrt((multi_touch_button_coords[2] - multi_touch_button_coords[0])*(multi_touch_button_coords[2] - multi_touch_button_coords[0])
			+ (multi_touch_button_coords[3] - multi_touch_button_coords[1])*(multi_touch_button_coords[3] - multi_touch_button_coords[1]));

		if (button == multi_touch_buttons[0])
		{
			multi_touch_button_coords[0] = x;
			multi_touch_button_coords[1] = y;
		}
		else if (button == multi_touch_buttons[1])
		{
			multi_touch_button_coords[2] = x;
			multi_touch_button_coords[3] = y;
		}

		float new_diag_len = sqrt((multi_touch_button_coords[2] - multi_touch_button_coords[0])*(multi_touch_button_coords[2] - multi_touch_button_coords[0])
			+ (multi_touch_button_coords[3] - multi_touch_button_coords[1])*(multi_touch_button_coords[3] - multi_touch_button_coords[1]));

		float H = window_h;
		float W = window_w;
		float full_diag = sqrt(W*W + H*H);
		if (!glut_viewer_is_enabled(GLUT_VIEWER_ROTATE_LIGHT)) {
			zoom = zoom * (1.0 + (new_diag_len - old_diag_len) / full_diag);
		}
...
void glut_viewer_main_loop(int argc, char** argv) {
    ...
    glutInitDisplayMode(GLUT_DOUBLE | GLUT_RGB | ( glut_viewer_is_enabled(GLUT_VIEWER_HDR) ? 0 : GLUT_DEPTH));
	...
	//multi-touch functions registration
	glutMultiButtonFunc(MultiTouchButton);
	glutMultiMotionFunc(MultiTouchMotion);
...
void glut_viewer_get_region_of_interest(
	float *xin, float *yin, float *zin, float *xax, float *yax, float *zax
	){

	*xin = xmin; *yin = ymin; *zin = zmin;
	*xax = xmax; *yax = ymax; *zax = zmax;
}
...
void mul_quats(float q1[4], float q2[4], float dest[4])
{

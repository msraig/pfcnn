...
namespace GlutViewerGUI {
    ...
    Widget::Widget(GLfloat x1, GLfloat y1, GLfloat x2, GLfloat y2) : style_(BlueStyle), visible_(true), highlight_(false) {
...
//______________________________________________________________________________________________________
	...
	//<<<<<
	void CurveEditor::set_curve(std::vector<GLfloat>& curve, bool update) {
		GLfloat vmin = 1e10, vmax = -1e10;
		for(unsigned int i=0; i<curve.size(); i++) {
			vmin = (vmin>curve[i])?curve[i]:vmin;
			vmax = (vmax<curve[i])?curve[i]:vmax;
        }
		if (curve.size()>CurveSize)
			for(unsigned int i=curve.size()-1; i>(curve.size()-CurveSize)-1; i--) {
				curve_[i+CurveSize-curve.size()] = (curve[i] - vmin)/(vmax - vmin);
			}
		else
			for(unsigned int i=0; i<curve.size(); i++) {
				curve_[i] = (curve[i] - vmin)/(vmax - vmin);
			}
...
//______________________________________________________________________________________________________
    ...
    ViewerProperties::ViewerProperties(GLfloat x_left, GLfloat y_top) : PropertyPage(
        ...
        if(glut_viewer_is_enabled(GLUT_VIEWER_HDR)) {
            add_slider("Exposure", exposure_, 0.001, 3.0) ;
            add_slider("Blur amount", blur_amount_) ;
            add_slider("Blur width", blur_width_, 1.0, 6.0) ;
        }
    }
    ...
    void ViewerProperties::apply() {
        glut_viewer_hdr_set_exposure(exposure_) ;
        glut_viewer_set_blur_amount(blur_amount_) ;
        glut_viewer_set_blur_width(blur_width_) ;
    }

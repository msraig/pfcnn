...
namespace Geex {
    ...
    GeexApp::GeexApp(int argc, char** argv) {
		...
		//glut_viewer_get_m_screen_size(&width_, &height_);
		//double ratio = (double)width_ / height_;	// w,h ratio
		//// adpatation version
		//height_ *= 0.93; // crop bottom
		//width_ = (int)(height_ * ratio);
		...
		std::cout << "CURRENT WINDOW SIZE: " << width_ << " " << height_ << std::endl;
...
//            fp_filtering_ = false ;
    ...
    void GeexApp::toggle_skybox_CB() {
        if(glut_viewer_get_skybox() == 0) {
            ...
            glut_viewer_set_vignette(1) ;
        } else {
            glut_viewer_set_skybox(0) ;
            glut_viewer_set_vignette(0) ;
        }
    }
    ...
    void GeexApp::main_loop() {
        ...
        viewer_properties_ = new GlutViewerGUI::ViewerProperties(400, 3000) ;
